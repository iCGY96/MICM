...
class VisionTransformer_MICM(nn.Module):
    """ Vision Transformer """
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,decoder_depth = 4,
        ...
        self.temp = nn.Parameter(torch.zeros(1, 1, embed_dim))
        ...
        self.decoder_head = torch.nn.Linear(embed_dim, 3 * patch_size ** 2)
        self.decoder_patch2img = Rearrange('(h w) b (c p1 p2) -> b c (h p1) (w p2)', p1=patch_size, p2=patch_size, h=img_size[0]//patch_size)
    ...
    def decoder_interpolate_pos_encoding(self, x, w, h):
        npatch = x.shape[1] - 1
    ...
    def prepare_tokens(self, x, mask=None):
        ...
        cls_tokens = self.temp.expand(B, -1, -1)
    ...
    def forward(self, x, return_all_tokens=None, mask=None, return_attention=False):
        ...
        ##decoder部分##################################################################
        B, np ,nc= x.shape
        w = int(math.sqrt(np)) * self.patch_embed.patch_size
        h = w
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        x = x + self.decoder_interpolate_pos_encoding(x, w, h)
        ...
        if(self.masked_im_modeling):
            ...
            patches = self.decoder_head(features)
            img = self.decoder_patch2img(patches)
        else:
            img = 0
        ##############################################################################
    ...
    def get_intermediate_layers(self, x, n=1, return_attention=False):
        ...
        x = x + self.decoder_interpolate_pos_encoding(x, w, h)
        for i, blk in enumerate(self.decoder_blocks):
            if len(self.decoder_blocks) - i <= n:
                x, attn = blk(x, return_attention=return_attention)
        ...
        if return_attention:
            return output, attns
        return output, None
    def get_intermediate_layers_withoutDec(self, x, n=1, return_attention=False):
        x = self.prepare_tokens(x)
        # we return the output tokens from the `n` last blocks
        output = []
        if return_attention:
...
######################################################################################
##########################################################################################
# class VisionTransformer_raw_mae(nn.Module):
#     """ Vision Transformer """
...
#                 x = blk(x)
#             else:
#                 x = blk(x, return_attention=return_attention)
...
# #####################################################################################
# #########################################################################################
# class VisionTransformer_feat_map(nn.Module):
#     """ Vision Transformer """
#     def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
#                  num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
#                  drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), return_all_tokens=False, 
...
#     ######################################################################################
#     ########################################################################################
# class VisionTransformer_clsToken_before(nn.Module):
#     """ Vision Transformer """
#     def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
#                  num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
#                  drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), return_all_tokens=False, 
...
#     def forward(self, x, return_all_tokens=None, mask=None, return_attention=False, rot_pred=False):
#         # mim
#         if self.masked_im_modeling:
#             assert mask is not None
#             x = self.prepare_tokens(x, mask=mask)
...
def vit_half_small(patch_size=16, **kwargs):
    model = VisionTransformer_MICM(
        patch_size=patch_size, embed_dim=384, depth=6, decoder_depth = 2, num_heads=6, mlp_ratio=4,
        qkv_bias=True, **kwargs)
    return model

# def vit_small(patch_size=16, **kwargs):
#     model = VisionTransformer_clsToken_before(
#         patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
#         qkv_bias=True, **kwargs)
#     return model

# def vit_small(patch_size=16, **kwargs):
#     model = VisionTransformer_raw_mae(
#         patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
#         qkv_bias=True, **kwargs)
#     return model

# def vit_small(patch_size=16, **kwargs):
#     model = VisionTransformer_feat_map(
#         patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
#         qkv_bias=True, **kwargs)
#     return model
