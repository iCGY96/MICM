...
from trans_test_tool import trans_test_gen
...
def feature_evaluation_cos(cl_data_file, n_way=5, n_support=5, n_query=15, adaptation=False):
    ...
    #z_support_proto = z_support.reshape(-1,z_support.size(-1))
    ...
    y = np.repeat(range(5), n_query)
    acc_b = (np.mean(pred == y) * 100)



    z_support_proto = z_support.reshape(-1,z_support.size(-1))
    ##############################################################
    z_support_proto, z_query = trans_test_gen(z_support_proto.cpu().detach().numpy(), z_query.cpu().detach().numpy(), n_shot = n_support)
    z_support_proto, z_query = torch.tensor(z_support_proto).cuda(),torch.tensor(z_query).cuda()

    ##############################################################
    cos_score = cos_sim(z_support_proto, z_query)
    pred = cos_score.cpu().numpy().argmax(axis=0)
    y = np.repeat(range(n_way), n_query)
    acc_a = (np.mean(pred == y) * 100)
...
def test_methods(args,server,epoch,pretrained_weights,file=None, top_k = 5, loss_type='softmax', evaluation_method='cosine', iter_num=2000):
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    torch.cuda.manual_seed_all(args.seed)
    random.seed(args.seed)

    n_query = 15 #600 - args.num_shots
    ...
    acc_all_b = [] ### ind result
    acc_all_a = [] ### transd result
    ...
    for i in range(iter_num):
        if evaluation_method == 'cosine':
            acc_b, acc_a = feature_evaluation_cos(cl_data_file, n_query=n_query, adaptation=False, **few_shot_params)
        ...
        acc_all_b.append(acc_b) ## before transductive
        acc_all_a.append(acc_a) ## after transductive
        if i % 50 == 0:
            print("%d steps reached and the mean acc is %g " % (
                i, np.mean(np.array(acc_all_a))))
                
    acc_mean1_b = np.mean(acc_all_b)
    acc_std1_b = np.std(acc_all_b)
    acc_mean1_a = np.mean(acc_all_a)
    acc_std1_a = np.std(acc_all_a)
    print('%d Test Acc at 100= %4.2f%% +- %4.2f%% / %4.2f%% +- %4.2f%%' % (iter_num, acc_mean1_b, 1.96 * acc_std1_b / np.sqrt(iter_num), acc_mean1_a, 1.96 * acc_std1_a / np.sqrt(iter_num))) ## inductive / transductive
    print(file_path)
    log_info = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
    log_info += '\n Epoch %d %d Shots Test Acc at %d= %4.2f%% +- %4.2f%% %s\n' % (
        epoch, args.num_shots, iter_num, acc_mean1_a, 1.96 * acc_std1_a / np.sqrt(iter_num), args.checkpoint_key)
